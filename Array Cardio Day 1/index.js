 // Get your shorts on - this is an array workout!
    ...
    const inventors = [
      ...
      const names = inventors.map(inventor => {return `${inventor.first} ${inventor.last}`})
      console.table(names)
      
      // Array.prototype.sort()
      // 3. Sort the inventors by birthdate, oldest to youngest

      const dates = inventors.sort((a,b) => a.year > b.year ? 1: -1)
      console.table(dates)
      ...
      const instances = data.reduce((allItems, currentItem) => {
            if(!allItems[currentItem]) {
                allItems[currentItem] = 0;
            }
